...
- **발행 파이프라인은 `scripts/publish.ts` 하나.** (tsx 실행, `src/lib`와 타입 공유)
  ...
  실측 글당 ~14K 토큰(입력 5.3K / 출력 8.6K). OpenAI 무료 한도 1M/일 = 약 71건.
  cron은 사이트당 8회 = 40건/일(한도의 약 56%)로, 글 길이 편차·재시도·계정 내
  다른 사용량을 감안한 안전 마진이다. 초과분은 표준 요금이며 luna보다 비싸므로,
  발행 횟수를 늘리려면 먼저 토큰 실측을 다시 해야 한다.
